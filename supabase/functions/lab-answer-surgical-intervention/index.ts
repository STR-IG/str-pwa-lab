...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const interventionTime = String(body?.interventionTime ?? "");
    const workShift = String(body?.workShift ?? "");
    const homeRest = String(body?.homeRest ?? "");
    ...
    const aiResponse = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        text: {
          ...
          format: {
            type: "json_schema",
            name: "surgical_intervention_guidance",
            strict: true,
            schema: {
              type: "object",
              properties: {
    ...
    if (!aiResponse.ok) {
      console.error("OpenAI surgical intervention guidance failed", aiResponse.status, aiData?.error?.code ?? "unknown");
    ...
    console.error("Unexpected lab-answer-surgical-intervention error", error instanceof Error ? error.message : "unknown");
